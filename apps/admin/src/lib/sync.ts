...
// ---- その日の合計トレーニング時間（分）----
// minutes>0 で保存（上書き）、0/未入力なら削除。
// userId を渡すとそのユーザーの分を対象にする（開発者が他ユーザーを編集する用途）。
// 省略時はログイン中の本人。他人分の書き込みはRLS（開発者のみ）で制御される。
...
// userId を渡すとそのユーザーの記録を対象にする（開発者が他ユーザーを編集する用途）。
